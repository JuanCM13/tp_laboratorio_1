...
int Get_nombre(char mensaje[], char mensajeError[], char cadena[], int rango)
{
    int ret;
    int respuesta;
...
int sonLetras (char cadena[], int rango)
{
    ...
    if(cadena != NULL && rango > 0)
    {
	    ...
	    {
	        if ( /*cadena[0] == 32 || */ !( (cadena[i] >= 'A' && cadena[i] <= 'Z') || (cadena[i] >= 'a' && cadena[i] <= 'z') || cadena[i] == 32) )
            {
	            ret = -2;
	            break;
	        }
...
int sonLetrasConGuion (char cadena[], int rango)
{
    int i;
    int ret;
...
int OrdenarNombre(char lista[], int rango)
{
    int ret;
    int i;
